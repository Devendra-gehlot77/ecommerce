...
export default function ProductListing() {
  ...
  useEffect(() => {
    axios
      .get("http://wscubetech.co/ecommerce-api/brands.php")
      .then((response) => {
                  ...
                  {categories.map((v, i) => {
                    return (
                      <FilterCategory
                        key={i}
                        data={v}
                        filterCategryData={filterCategryData}
                        allFilterCategory={allFilterCategory}
                      />
                    );
                  })}
                </div>
                  ...
                  {brand.map((v, i) => {
                    return (
                      <FilterBrand
                        key={i}
                        data={v}
                        filterBrandyData={filterBrandyData}
                        allFilterBrand={allFilterBrand}
                      />
                    );
                  })}
                </div>
                ...
                <div>
                  ...
                  <div className="col-12  order-md-1 order-3 mt-2 mt-md-0">
                    <div className="input-group  mb-2 ">
                      ...
                      <input
                        type="text"
                        className="form-control border-start-0 search-items   "
  ...
  return (
    <div className="form-check mb-2">
      <input
        ...
        onChange={() => filterCategryData(data.slug)}
        type="checkbox"
        checked={allFilterCategory.includes(data.slug) ? "checked" : ""}
        id={data.slug}
      />
      <label className="form-check-label" htmlFor={data.slug}>
        {data.name}
      </label>
    </div>
  );
...
var FilterBrand = ({ data, filterBrandyData, allFilterBrand }) => {
  return (
    <div className="form-check mb-2">
      <input
        className="form-check-input"
        onChange={() => filterBrandyData(data.slug)}
        checked={allFilterBrand.includes(data.slug) ? "checked" : ""}
        type="checkbox"
        id={data.slug}
      />
      <label className="form-check-label" htmlFor={data.slug}>
